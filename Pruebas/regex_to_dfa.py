...
class RegexToDFA:
    def __init__(self, regex):
        # Envolver cualquier literal entre comillas con paréntesis
        if isinstance(regex, str) and len(regex) > 2 and regex[0] in ["'", '"'] and regex[0] == regex[-1]:
            regex = f"({regex[1:-1]})"
        
        self.original_regex = regex
        self.augmented_regex = f"({regex})#"
        
        # Manejar literales más robustamente
        self.alphabet = set(re.findall(r'[a-zA-Z0-9_+\-*/()[\]{}]', regex))
    ...
    def infix_to_postfix(self, regex):
        # Implementación mejorada del algoritmo Shunting Yard
        output = []
        stack = []
        ...
        for i in range(len(regex) - 1):
            ...
            # Reglas mejoradas para concatenación
            if (regex[i] not in '(|' and 
                regex[i+1] not in ')|*+?·' and 
                not (regex[i] in ')' and regex[i+1] in '(') and
                not (regex[i].isalnum() and regex[i+1].isalnum())):
